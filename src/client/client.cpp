    ...
    exchange_min(0), exchange_max(1), discard_num(0)
{
    ...
    callbacks[S_COMMAND_BATTLE_START] = &Client::startBattle;
    ...
    callbacks[S_COMMAND_UPDATE_GENERAL_PILE] = &Client::updateGeneralPile;
    ...
    interactions[S_COMMAND_SKILL_TRANSFERFIELDCARDS] = &Client::askForTransferFieldCard;
    ...
    callbacks[S_COMMAND_CLEAR_GENERAL_BOX] = &Client::clearGB;
...
void Client::updateProperty(const QVariant &arg)
{
    ...
    if (args[1] == "phase") {
        ...
        if (player->getPhase() == Player::Finish) {
            if (player->hasFlag("shuangxiong_postpone") && player == Self && !Self->ownSkill("shuangxiong"))
...
void Client::startBattle(const QVariant &)
{
    emit battle_started();
}
...
QString Client::getPlayerName(const QString &str, bool full_name)
{
    ...
    if (rx.exactMatch(str)) {
        ...
        if (full_name) {
            ...
            return Sanguosha->translate(QString("SEAT(%1)").arg(QString::number(player->property("UI_Seat").toInt())));
    } else
...
QString Client::setPromptList(const QStringList &texts)
{
    ...
    if (texts.length() >= 6) {
        QString arg3 = Sanguosha->translate(texts.at(5));
        prompt.replace("%arg3", arg3);
    }
...
void Client::onPlayerChooseCard(const QList<int> &card_ids)
{
...
void Client::askForGeneral(const QVariant &arg)
{
    ...
    bool same_kingdom = true;
    if (args.length() > 3)
        same_kingdom = args[3].toBool();
...
void Client::clearGB(const QVariant &)
{
    emit gb_cleared();
}
...
void Client::askForPlayerChosen(const QVariant &players)
{
    ...
    QStringList qiaobianPattern;
    if (args.size() >= 7 && JsonUtils::isBool(args[6]) && JsonUtils::isBool(args[7])) {
        if (args[6].toBool())
            qiaobianPattern << "equipArea";
        if (args[7].toBool())
            qiaobianPattern << "judgingArea";
    }
    _m_roomState.setCurrentCardUsePattern(qiaobianPattern.join(","));
...
void Client::updateGeneralPile(const QVariant &arg)
{
    JsonArray args = arg.value<JsonArray>();
    if (args.size() != 4 || !JsonUtils::isString(args[0]) || !JsonUtils::isString(args[1]) || !JsonUtils::isBool(args[3]))
        return;

    ClientPlayer *who = getPlayer(args[0].toString());
    QString pile_name = args[1].toString();
    QStringList general_names = args[2].toStringList();
    if (!args[3].toBool() && who != Self) {
        QStringList null_names;
        for (int i = 0; i < general_names.length(); i++) {
            null_names.append("unknown");
        }
        general_names = null_names;
    }

    if (who)
        who->changeGeneralPile(pile_name, general_names);
}
